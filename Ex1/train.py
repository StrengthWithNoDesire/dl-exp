...
def main():
    ...
    X_train_val, X_test, y_train_val, y_test = train_test_split(X, y, test_size=0.3, random_state=rd)
    X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val, test_size=0.2, random_state=rd)
    print("数据集划分完成")

    model = Model(learning_rate=learning_rate, optimizer=optimizer)
    model.add_layer(layer_input_dim=784, layer_output_dim=256, initialize='xavier', activation='relu', l2=l2_reg, l2_lambda=l2_lambda)
    model.add_layer(layer_input_dim=256, layer_output_dim=128, initialize='xavier', activation='relu', l2=l2_reg, l2_lambda=l2_lambda)
    model.add_layer(layer_input_dim=128, layer_output_dim=10, initialize='xavier', activation='linear', l2=l2_reg, l2_lambda=l2_lambda)
    ...
    y_test_pred_probs = softmax(y_test_pred_logits)
    y_test_pred = np.argmax(y_test_pred_probs, axis=1)

    test_loss = cross_entropy_loss(y_test, y_test_pred_probs)
    if l2_reg:
        l2_loss = sum(l2_regularization_loss(layer.W, l2_lambda) for layer in model.layers)
        test_loss += l2_loss
    y_test_true_labels = np.argmax(y_test, axis=1)
    test_accuracy = np.mean(y_test_pred == y_test_true_labels)

    plot_confusion_matrix(y_test_true_labels, y_test_pred)
